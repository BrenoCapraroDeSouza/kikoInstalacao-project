import { ElipseAna, ElipseBreno, ElipseCarlos, ElipseMaria } from '../../assets'
...
const Depoiments = () => {
  ...
  return (
    <section className='Depoiments'>
        <h2 className='DepoimentsTitle'><span className="orange">Experiências</span> de nossos <span className="orange">CLIENTES</span></h2>
        <div className="gallery-depoiments">
            {DepoimentsArray.map((depoiment) =>{
                return <Depoiment key={depoiment.id} type={depoiment.type} perfilImg={depoiment.perfilImg} author={depoiment.author} text={depoiment.text}/>
            })} 
        </div>
    </section>
  )
